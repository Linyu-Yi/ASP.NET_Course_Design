...
public partial class m_putLoad : System.Web.UI.Page
{
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        {
            this.ClientScript.RegisterStartupScript(this.GetType(), "1", "<script type=\"text/javascript\">alert(\"对不起！没有找到您输入信息的相关记录，请检查是否输入有误！\");</script>");
        }
        else
        {
            ...
            {
                this.ClientScript.RegisterStartupScript(this.GetType(), "1", "<script type=\"text/javascript\">alert(\"对不起！您查询的券已经过期了，不能在提货了，如有问题请联系客服！\");</script>");
            }
            else
            {
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (iList == null)
        {
            this.ClientScript.RegisterStartupScript(this.GetType(), "1", "<script type=\"text/javascript\">alert(\"对不起！礼品列表为空不能提货！\");</script>");
            return;
        }
        foreach (cart_items item in iList)
        {
            ...
            if (myt.status == 3)
            {
                this.ClientScript.RegisterStartupScript(this.GetType(), "1", "<script type=\"text/javascript\">alert(\"对不起！您的礼券已经被领取不能重复领取！\");</script>");
                return;
            }
        }
        //保存订单=======================================================================
        ...
        if (model.Add() == 0)
        {
            this.ClientScript.RegisterStartupScript(this.GetType(), "1", "<script type=\"text/javascript\">alert(\"订单保存过程中发生错误，请重新提交！\");</script>");
            return;
        }
        ...
        this.ClientScript.RegisterStartupScript(this.GetType(), "1", "<script type=\"text/javascript\">alert(\"恭喜您！订单提交成功！我们会尽快给您安排发货，请注意查收！发货信息将会发送到您的手机上！请您留意！\");</script>");
    }
}