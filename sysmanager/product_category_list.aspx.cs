...
public partial class sysmanager_product_category_list : ManagePage
{
    ...
    {
        //检查权限
        if (ChkAdminLevel(this.Page, 30, "Delete"))
        {
            // 当前点击的按钮
            ...
            if (!bllpp.ExistsKD(caId))//查找是否存在该快递已经有发货记录
            {
                bll.Delete(caId);
                AddAdminLog("删除", "删除快递公司：" + title + ""); //记录日志
                JscriptMsg(this.Page, " 成功删除快递公司：" + title + "", Utils.CombUrlTxt("product_category_list.aspx", "keywords={0}&page={1}", this.keywords, this.page.ToString()), "Success");
            }
            else
            {
                JscriptMsg(this.Page, "该快递公司已经有发货记录，不能删除！", "", "Error");
                return;
            }
        }
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        JscriptMsg(this.Page, " 排序保存成功", Utils.CombUrlTxt("product_category_list.aspx", "keywords={0}&page={1}", this.keywords, this.page.ToString()), "Success");
    }
}
