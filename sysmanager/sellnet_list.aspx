<%@ Page Language="C#" AutoEventWireup="true" CodeFile="sellnet_list.aspx.cs" Inherits="sysmanager_sellnet_list" %>
...
<body>
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <a href="sellnet_edit.aspx?action=Add"><li><span><img src="../images/t01.png" /></span>添加</li></a>
    ...
    <dl class="seachform">
       <dd><label>销售网点名称</label><span class="single-select"><asp:TextBox ID="txtKeywords" runat="server" CssClass="scinput" Width="300"></asp:TextBox></span></dd>
...
<ItemTemplate> 
      ...
      <asp:HiddenField ID="hidId" Value='<%#Eval("id")%>' runat="server" /></td>
		...
		<td><asp:TextBox ID="txtSortId" runat="server" Height="25"  Text='<%#Eval("sort_id")%>' CssClass="scinput" Width="40" onkeydown="return checkNumber(event);" /></td>
     	<td><%# Convert.ToInt32(Eval("status")) == 2 ? "<font color=red>不可用</font>" : "可用"%></td>
	    <td><a href="sellnet_edit.aspx?action=Edit&id=<%#Eval("id")%>&page=<%=page%>" class="tablelink"><font color =green>[修改]</font></a>  &nbsp;&nbsp;<asp:LinkButton ID="lbtnDelCa" runat="server" CommandArgument='<%# Eval("id")%>' OnClientClick="return confirm('是否真的要删除？')" onclick="lbtnDelCa_Click"><font color =red>[删除]</font></asp:LinkButton></td>
        </tr> 
  </ItemTemplate>
<FooterTemplate>
