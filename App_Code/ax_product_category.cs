...
using System.Data.SqlClient;
   ...
   /// 快递公司-类
	/// </summary>
	[Serializable]
	public partial class ax_product_category
	{
		public ax_product_category()
		{}
        #region Model
        private int _id;
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from [ax_product_category]");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 title from [ax_product_category]");
            strSql.Append(" where id=" + id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 remark from [ax_product_category]");
            strSql.Append(" where id=" + id);
            string title = Convert.ToString(DbHelperSQL.GetSingle(strSql.ToString()));
            if (string.IsNullOrEmpty(title))
            {
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into [ax_product_category] (");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [ax_product_category] set ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update ax_product_category set " + strValue);
            strSql.Append(" where id=" + id);
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from [ax_product_category] ");
			strSql.Append(" where id=@id ");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
			parameters[0].Value = id;
		...
		public void GetModel(int id)
		{
			...
			strSql.Append(" FROM [ax_product_category] ");
			strSql.Append(" where id=@id ");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
			parameters[0].Value = id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from ax_product_category");
            if (category_id == 0)
            {
