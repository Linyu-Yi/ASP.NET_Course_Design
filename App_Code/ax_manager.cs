...
using System.Data.SqlClient;
	...
	/// </summary>
	[Serializable]
	public partial class ax_manager
	{
		public ax_manager()
		{}
        #region Model
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from [ax_manager]");
        ...
        /// 是否存在角色该记录
        /// </summary>
        public bool ExistsRole(int role_id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [ax_manager]");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 user_name from [ax_manager]");
            strSql.Append(" where id=" + id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 real_name from [ax_manager]");
            strSql.Append(" where id=" + id);
            string title = Convert.ToString(DbHelperSQL.GetSingle(strSql.ToString()));
            if (string.IsNullOrEmpty(title))
            {
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into [ax_manager] (");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [ax_manager] set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from [ax_manager] ");
			strSql.Append(" where id=@id ");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
			parameters[0].Value = id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM  ax_manager");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
